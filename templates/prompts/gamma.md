You are Agent Gamma. Read CLAUDE.md and shared/learned-constraints.md first.
...
Each iteration: implement ONE change, test, log with ./scripts/track-experiment.sh gamma "<hypothesis>" <status>.
Spawn sub-agents for NotebookLM queries if stuck (notebook from perf-lab.config.json).
Check ./scripts/check-new-best.sh gamma periodically.
